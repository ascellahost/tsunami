...
pub fn command() -> twilight_model::application::command::Command {
    CommandBuilder::new(
        Lang::fallback().stats_name().into(),
        Lang::fallback().stats_desc().into(),
...
pub struct AscellaStats {
    ...
    /// The amount of domains ascella has use the domains endpoint to find out
    /// which
    #[openapi(example = "17")]
...
impl AscellaStats {
    pub fn new(total_uploads: i64, total_domains: i64, total_users: i64, total_views: i64, day_uploads: i64) -> Self {
        ...
        Self {
            ...
            uptime: format_duration(Instant::now().duration_since(*START_TIME.get().unwrap_or(&Instant::now())))
                .to_string(),
            fast: true,
...
pub async fn execute(_client: &Client, _cmd: &ApplicationCommand) -> Result<BotResponse> {
    ...
    let embed = create_embed()
        ...
        .field(
            EmbedFieldBuilder::new(
                "Commit Hash",
                format!("[{}]({})", &env!("GIT_HASH")[..7], &stats.commit_hash),
            )
            .inline(),
        )
        .build();
