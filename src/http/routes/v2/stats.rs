use crate::{
    database::s3::S3,
    http::models::stats::{DisplayEmbed, StatsResponse},
    prelude::*,
};
...
pub async fn get(image: web::Path<String>) -> Result<OkResponse<StatsResponse>, Error> {
    ...
    if let Ok(image) = data {
        if let Ok(user) = get_user::exec(image.owner).await {
            ...
            // Updates the view count in a non blocking way this endpoint gets called on
            // every view
            ...
            if let Ok(rembed) = embed {
                let replacers = [
                    ...
                    (
                        "%TOTAL_IMAGES%",
                        get_user_image_count::exec(user.id)
                            .await
                            .unwrap_or_default()
                            .to_string(),
                    ),
                ...
                // you can get the embed with get_embed::exec and then create a new DisplayEmbed
                // and set that embed on the json
